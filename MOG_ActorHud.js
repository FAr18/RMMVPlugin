...
Actor_Hud.prototype.create_states = function() {
	...
	this._state_stacks = new Sprite();//added
	this._state_stacks.bitmap = new Bitmap(Window_Base._iconWidth, Window_Base._iconHeight);//added
	this._state_stacks.x = this._state_icon.x;//added
	this._state_stacks.y = this._state_icon.y;//added
	this.addChild(this._state_icon);
	this.addChild(this._state_stacks);//added
	this.refresh_states();	
};
...
Actor_Hud.prototype.refresh_states = function() {
	//._state_date[0] :目前顯示的iconId
	//._state_date[1] :目前顯示狀態的索引值
	//._state_date[2] :顯示狀態的切換計數器
	this._states_data[0] = 0;
	this._states_data[2] = 0;
	this._state_icon.visible = false;
	var icons = this._battler.allUniqueIcons();
	...
	if (icons[this._states_data[1]]) {
		...
		var count = this._battler.getStateCount(states[this._states_data[1]].id);
		this._state_stacks.bitmap.clear();
		if (count > 1) {
			this._state_stacks.bitmap.drawText(count, 0, 0, Window_Base._iconWidth, Window_Base._iconHeight, 'center');
		}
